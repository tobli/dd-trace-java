...
class DDEvpProxyApiTest extends DDCoreSpecification {
  ...
  def "sending an empty list of traces returns no errors"() {
    setup:
    def evpProxyEndpoint = "evp_proxy/v2/"
  ...
  def "content is sent as MSGPACK"() {
    ...
    agentEvpProxy.lastRequest.contentType == "application/msgpack"
    convertMap(agentEvpProxy.lastRequest.body) == expectedRequestBody
  ...
  def createEvpProxyApi(String agentUrl, String evpProxyEndpoint, TrackType trackType) {
    ...
    return DDEvpProxyApi.builder().agentUrl(hostUrl).evpProxyEndpoint(evpProxyEndpoint).trackType(trackType).build()
  }
  ...
  def buildAgentEvpProxyPath(String evpProxyEndpoint, TrackType trackType, String apiVersion) {
    return String.format("%s/api/%s/%s", evpProxyEndpoint, apiVersion, trackType.name().toLowerCase())
  }
